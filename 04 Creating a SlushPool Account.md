...
There are several different pools to choose from but the basic idea is the same with most of them. You will create an account, provide a bitcoin deposit address, and then copy/paste the mining pool URL into your ASIC configuration file so that your hash power is pointed there. Mining pools typically retain a small fee of your rewards between 1% - 3%. Payout methods also vary from pool to pool, SlushPool uses a Pay Per Last N Shares (PPLNS) method, you receive rewards based on the number of blocks the pool finds and your proportional average contribution to the pool during those block finds. You can learn more about mining pools [here](https://miningpools.com/bitcoin/).
